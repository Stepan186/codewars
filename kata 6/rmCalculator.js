...
// Since you are not sure which formula to use and you are feeling confident, your function will return the largest value from the three formulas shown above, rounded to the nearest integer. However, if the number of repetitions passed in is 1 (i.e., it is already a one rep max), your function must return w. Also, if the number of repetitions passed in is 0 (i.e., no repetitions were completed), your function must return 0.


function calculate1RM(w, r){
    let epley = Math.round(w * (1 + r/30))
    let mcGlothin = Math.round(100 * w / (101.3 - 2.67123 * r))
    let lombardi = Math.round(w * Math.pow(r, 0.10))
    if(r === 1) {
        return w
    }
    else if (r === 0) {
    ...
    return Math.max(epley, mcGlothin, lombardi)
}
